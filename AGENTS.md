...
- Claude hook ingress: `Prototype/Sources/IslandBridge/`, `PingIsland/Services/Hooks/HookInstaller.swift`, `PingIsland/Services/Hooks/HookSocketServer.swift`
  ...
  - Remote state delivery uses processed acknowledgements and a bounded persisted outbox. Acknowledge only after async store ingestion completes; acknowledge filtered events so they do not poison replay; decisions also acknowledge their responsive request. Keep app and remote bridge protocol changes atomic and backwards-compatible with missing optional metadata.
...
- If you change session lifecycle or transitions, start in `SessionStore`. Avoid ad-hoc state mutation elsewhere.
  ...
  - Remote ingress has a foreign process namespace: never inspect, reap, deduplicate, or supersede a remote session by applying a PID to the local Mac. Disconnection removes execution and manual-attention eligibility without manufacturing a provider end event; a matching endpoint can reconnect the same session.
...
- If you change built-in notification sounds or startup audio, inspect `PingIsland/Core/Settings.swift`, `PingIsland/Core/SoundPackCatalog.swift`, `PingIsland/UI/Views/SettingsWindowView.swift`, `PingIsland/App/AppDelegate.swift`, and `PingIsland/Resources/Sounds/` together so mode selection, fixed mappings, previews, and bundled assets stay aligned.
  ...
  - All notification sounds route through `SoundPlaybackCoordinator`. `NotificationAudioOutputResolver` snapshots the output device already alive and selected on the Mac, then assigns that explicit UID to `NSSound`; this preserves AirPods already connected to the Mac without allowing playback startup to request an automatic Bluetooth handoff. An unavailable current device may fall back only to a built-in speaker terminal, otherwise playback is skipped. Never leave the playback identifier nil, and return before resolving a device when sounds are disabled or volume is zero.
...
- If you change completion-result popup behavior, trace through `SessionStore`, `SessionMonitor`, `PingIsland/UI/Views/NotchView.swift`, and `PingIsland/UI/Views/SessionCompletionNotificationView.swift` so completion detection, queueing, and auto-dismiss timing stay aligned.
  ...
  - A blocked completion remains queued by its captured key and immutable session snapshot until a surface can claim it. Opening, pinning, or another active session must not consume the key, and later activity on the same session must not overwrite the queued completed turn. Keep the pending queue shared across docked/detached surface recreation.
