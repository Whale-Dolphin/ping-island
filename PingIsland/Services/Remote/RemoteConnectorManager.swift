...
final class RemoteConnectorManager: ObservableObject {
        ...
        case .hookEvent(let eventMessage):
            ...
            if payload.expectsResponse,
               let decision = Self.immediateRemoteCodexPermissionDecision(
                ...
                // Only an explicit bypassPermissions PermissionRequest uses the
                // existing approval decision. auto_review stays a nil-decision
                // deferral, sent to this remote request rather than a local socket.
    ...
    nonisolated static func immediateRemoteCodexPermissionDecision(
        provider: String, eventType: String, permissionMode: String?, approvalsReviewer: String?
    ) -> String? {
        if isRemoteCodexBypassPermissionRequest(provider: provider, eventType: eventType, permissionMode: permissionMode) {
            return "approve"
        }
        return shouldDeferRemoteCodexApproval(
            provider: provider, eventType: eventType, permissionMode: permissionMode, approvalsReviewer: approvalsReviewer
        ) ? "defer" : nil
    }
