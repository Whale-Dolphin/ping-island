...
final class RemoteHookConfigurationTests: XCTestCase {
    ...
    func testRemoteCodexImmediateResponsesRespectPermissionRequestAndReviewer() {
        XCTAssertEqual(RemoteConnectorManager.immediateRemoteCodexPermissionDecision(
            provider: "codex", eventType: "PermissionRequest", permissionMode: "bypassPermissions", approvalsReviewer: nil
        ), "approve")
        XCTAssertEqual(RemoteConnectorManager.immediateRemoteCodexPermissionDecision(
            provider: "codex", eventType: "PermissionRequest", permissionMode: "default", approvalsReviewer: "auto_review"
        ), "defer")
        for event in ["PreToolUse", "PostToolUse", "Stop", "UserPromptSubmit"] {
            ...
            XCTAssertNil(RemoteConnectorManager.immediateRemoteCodexPermissionDecision(
                provider: "codex", eventType: event, permissionMode: "bypassPermissions", approvalsReviewer: "auto_review"
            ))
        }
        for reviewer in [nil, "user", "guardian_subagent"] as [String?] {
            XCTAssertNil(RemoteConnectorManager.immediateRemoteCodexPermissionDecision(
                provider: "codex", eventType: "PermissionRequest", permissionMode: "default", approvalsReviewer: reviewer
            ))
        }
    }
    ...
    func testOlderBridgePayloadWithoutReviewerMetadataStillDecodes() throws {
        ...
        XCTAssertNil(payload.approvalsReviewer)
        XCTAssertFalse(RemoteConnectorManager.shouldDeferRemoteCodexApproval(
            provider: payload.provider, eventType: payload.event,
            permissionMode: payload.permissionMode, approvalsReviewer: payload.approvalsReviewer
        ))
