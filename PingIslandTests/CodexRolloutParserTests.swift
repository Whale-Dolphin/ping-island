...
final class CodexRolloutParserTests: XCTestCase {
    func testReviewerInferenceSurvivesBatchesAndIncrementalSettingsChanges() async throws {
        ...
        {"type":"turn_context","payload":{"turn_id":"turn-1","approval_policy":"on-request","approvals_reviewer":" AUTO-REVIEW "}}
        """
        let padding = Array(repeating: "{\"type\":\"event_msg\",\"payload\":{\"type\":\"token_count\"}}", count: 140).joined(separator: "\n")
        let call = """
        ...
        var snapshot = await parser.parseThread(threadId: threadId, fallbackCwd: "/tmp/reviewer-project", clientInfo: client)
        ...
        for (reviewer, phase) in [("guardian_subagent", SessionPhase.waitingForInput), ("auto_review", .processing)] {
            try handle.write(contentsOf: Data("{\"type\":\"turn_context\",\"payload\":{\"turn_id\":\"turn-1\",\"approvalsReviewer\":\"\(reviewer)\"}}\n".utf8))
            snapshot = await parser.parseThread(threadId: threadId, fallbackCwd: "/tmp/reviewer-project", clientInfo: client)
            XCTAssertEqual(snapshot?.phase, phase)
            XCTAssertEqual(snapshot?.intervention != nil, phase == .waitingForInput)
        }
        // Missing reviewer metadata on another turn is unknown, not inherited auto_review.
    ...
    func testNeverPolicyDoesNotInferMCPHumanApproval() async throws {
        let directory = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: directory) }
        let file = directory.appendingPathComponent("rollout.jsonl")
        try """
        {"type":"session_meta","payload":{"id":"never-policy","cwd":"/tmp/reviewer-project","source":"cli"}}
        {"type":"turn_context","payload":{"turn_id":"turn-1","approval_policy":" NEVER ","approvals_reviewer":"user"}}
        {"type":"response_item","payload":{"type":"function_call","name":"mcp__test__read","arguments":"{}","call_id":"call-1"}}
        """.write(to: file, atomically: true, encoding: .utf8)
        let parser = CodexRolloutParser()
        let snapshot = await parser.parseThread(
            threadId: "never-policy", fallbackCwd: "/tmp/reviewer-project",
            clientInfo: SessionClientInfo(kind: .codexCLI, sessionFilePath: file.path)
        )
        XCTAssertEqual(snapshot?.phase, .processing)
        XCTAssertNil(snapshot?.intervention)
    }
